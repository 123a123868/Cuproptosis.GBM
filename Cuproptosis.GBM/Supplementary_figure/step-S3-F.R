...
identical(colnames(expr.tumor),pd.CC$sample)
save(pd.CC,file = "Supplementary_figure_3/out_data/pd_cc.Rdata")

# 读入免疫细胞浸润数据
gmtFile <- "Input_data/Immune_cell_infiltration//immune.gmt"   
dimnames <- list(rownames(expr.tumor),colnames(expr.tumor))
mat <- matrix(as.numeric(as.matrix(expr.tumor)),nrow=nrow(expr.tumor),dimnames=dimnames)
mat <- avereps(mat)
...
# save data----
write.table(ssgseaOut,file="Supplementary_figure_3//out_data/ssgseaOut.txt",
...
rm(list = ls())  
options(stringsAsFactors = F) 
input <- "Supplementary_figure_3//out_data/ssgseaOut.txt"  
immune <- read.table( input ,sep="\t",header=T,row.names=1,check.names=F)
immune[1:4, 1:4]
load("Supplementary_figure_3//out_data/pd_cc.Rdata")
...
pdf("Supplementary_figure_3//out_polt/S3_F.pdf",height=7,width=15)
